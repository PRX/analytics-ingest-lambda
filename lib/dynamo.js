...
exports.get = async (ids, idField = 'id') => {
  ...
  const iter = new BatchGet(dynamoDb, keys);
  while (true) {
  ...
  const keyedResults = {}
  await Promise.all(results.map(async ([tbl, data]) => {
    const id = data.id.S;
...
exports.write = async (records, idField = 'id') => {
  ...
  // TODO: async iterators not supported in node 8.10
  let numDone = 0;
  const iter = new BatchWrite(dynamoDb, Object.values(keys));
  while (true) {
    const result = await iter.next();
    if (result.done === false) {
      numDone++;
...
exports.delete = async (ids) => {
  ...
  // TODO: async iterators not supported in node 8.10
  let numDone = 0;
  const iter = new BatchWrite(dynamoDb, keys);
  while (true) {
    const result = await iter.next();
    if (result.done === false) {
      numDone++;
