...
import { toEpochMilliseconds, toEpochSeconds } from "./timestamp";
...
export async function addFrequency({ client, listener, campaign, timestamp, maxSeconds } = {}) {
  ...
  const command = new UpdateItemCommand(params);
  const result = await (client || (await getClient())).send(command);
  // return new DynamoFrequency({ maxSeconds, result });
  return { maxSeconds, result };
}
...
export async function removeFrequency({ client, listener, campaign, timestamps } = {}) {
  const params = {
    ...
    ExpressionAttributeValues: {
      ":td": { NS: timestamps.map((td) => `${toEpochMilliseconds(td)}`) },
    },
  };

  const command = new UpdateItemCommand(params);
  const result = await (client || (await getClient())).send(command);
  // return new DynamoFrequency({ result });
  return { result };
}
