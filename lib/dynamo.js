...
exports.updateResult = async (id, setPayload, setSegments, setExtras, result) => {
  ...
  const oldExtras =
    result.Attributes && result.Attributes.extras ? result.Attributes.extras.S : null;
  ...
  if (newPayload && setExtras) {
    Object.keys(setExtras).forEach(key => (newPayload[key] = setExtras[key]));
  } else if (newPayload && oldExtras) {
    try {
      const extras = JSON.parse(oldExtras);
      Object.keys(extras).forEach(key => (newPayload[key] = extras[key]));
    } catch (err) {
