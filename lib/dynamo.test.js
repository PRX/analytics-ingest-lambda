...
describe("dynamo", () => {
  ...
  describe(".removeFrequency", () => {
    it("sets params", async () => {
      ...
      const now = Date.now();
      await dynamo.removeFrequency({ listener: "l1", campaign: "c2", timestamps: [3, 4, now] });
      ...
      expect(ddbMock).toHaveReceivedCommandWith(UpdateItemCommand, {
        ...
        // NOTE: these do not get translated to milliseconds, so we're backwards compatible
        // with deleting epoch-seconds
        ExpressionAttributeValues: { ":td": { NS: ["3", "4", now.toString()] } },
      });
    });
  });
