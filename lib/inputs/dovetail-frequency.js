 ...
 */
export default class DovetailFrequency {
  constructor(records) {
  ...
  async insert() {
    ...
    await this.eachImpression(async (rec, imp) => {
      if (!imp.isDuplicate && this.checkFrequency(imp)) {
      ...
      while (args) {
        try {
          ...
          if (toDelete.length > 0) {
            ...
            const _deleteResult = await dynamo.updateItemPromise(deleteParams, client);
            deleteCount++;
          }
          ...
          if (err.name === "ProvisionedThroughputExceededException") {
            logger.warn(`DDB throughput exceeded [${process.env.DDB_FREQUENCY_TABLE}]: ${err}`, {
              args,
            });
          } else {
  ...
  minImpressionTime(data) {
    return Math.floor(Date.now() / 1000 - data.maxSeconds);
  }

  evictImpressions(data, result) {
    let toEvict = [];
    if (result.Attributes?.impressions) {
