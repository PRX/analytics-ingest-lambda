...
module.exports = class RedisIncrements {
  ...
  check(record) {
    if (record.feederPodcast || record.feederEpisode) {
      return (record.type === 'combined' && record.download && !record.download.isDuplicate)
          || (record.type === 'download' && !record.isDuplicate);
    } else {
      return false;
    }
  }
