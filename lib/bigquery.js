...
export function client(
  credentials = process.env.BQ_CREDENTIALS,
  config = process.env.BIGQUERY_CLIENT_CONFIG,
) {
  if (config) {
    try {
      config = JSON.parse(config);
    } catch (_err) {
      throw new Error(`Invalid BQ config: ${config}`);
    }
    return new BigQuery({
      credentials,
      projectId:
        credentials.project_id || credentials.audience.match(/projects\/([0-9]+)\/locations/)[1],
    });
  }
  // Add after everything has moved to federated config
  // else {
  //   throw new Error("Missing BQ Config");
  // }

  // Remove after everything has moved to federated config
  if (credentials) {
    try {
