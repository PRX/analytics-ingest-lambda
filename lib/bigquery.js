 ...
 * Streaming inserts
 */
export const insert = async (dataset, table, rows, retries = 2) => {
  if (rows.length === 0) {
    return 0;
  }
  try {
    const client = await client();
    const _result = await client.dataset(dataset).table(table).insert(rows, { raw: true });
    return rows.length;
  } catch (err) {
    if (err.message.match(/client_email/)) {
      ...
      return insert(dataset, table, rows, retries - 1);
    } else {
      throw err;
    }
  }
...
export const key = async (force = false) => {
  if (_key && !force) {
    return _key;
  } else {
