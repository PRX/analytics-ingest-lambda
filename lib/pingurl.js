...
const settings = {
  keepAlive: true,
  keepfreeSocketTimeout: 5000,
  maxSockets: 600,
  maxFreeSockets: 10
};
const httpAgent = new Agent(settings);
const httpsAgent = new Agent.HttpsAgent(settings);

// for testing only - gracefully close sockets
...
exports.ping = async (pingUrl, inputData, timeout, timeoutWait) => {
  ...
  for (let i = 1; i <= MAX_ATTEMPTS; i++) {
    try {
      ...
      } else if (i < MAX_ATTEMPTS && err.reusedSocket) {
        logger.warn(`REUSEDSOCKET ${opts.url}`);
        await new Promise(resolve => setTimeout(resolve, timeoutWait));
