...
exports.test = async record => {
  ...
  if (record.download && record.download.isDuplicate) {
    ...
    if (record.download?.cause !== 'domainthreat') {
      ...
      const remoteReferrer = record.remoteReferrer;
      logger.warn('MISMATCHED domainthreat', { analytics, router, remoteReferrer });
    }
    ...
    if (record.download?.cause !== `datacenter: ${center.provider}`) {
      ...
      logger.warn(`MISMATCHED datacenter`, { analytics, router, remoteIp });
    }
