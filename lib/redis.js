...
module.exports = class Redis {
  ...
  static impressions(dtim) { return `impression:${timestamp.toDateString(dtim)}:actuals`; }
  ...
  static buildConn(redisUrl, maxRetries) {
    ...
    if (redisUrl.match(/^cluster:\/\/.+:[0-9]+$/)) {
      ...
      return new IORedis.Cluster(config, {clusterRetryStrategy: opts.retryStrategy, redisOptions: opts});
    } else {
