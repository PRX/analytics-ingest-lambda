...
describe('dovetail-impressions', () => {
  ...
  it('creates unique insert ids for ads', async () => {
    const r1 = await impression.format({impressions: [{adId: 1}], listenerEpisode: 'req1'});
    const r2 = await impression.format({impressions: [{adId: 1}], listenerEpisode: 'req1'});
    const r3 = await impression.format({impressions: [{adId: 2}], listenerEpisode: 'req1'});
    const r4 = await impression.format({impressions: [{adId: 1}], listenerEpisode: 'req2'});
  ...
  it('inserts impression records', () => {
    ...
    return impression2.insert().then(result => {
      ...
      expect(inserts['dt_impressions'][0].json.listener_episode).to.equal('listen2');
      expect(inserts['dt_impressions'][0].json.ad_id).to.equal(1);
      expect(inserts['dt_impressions'][1].json.listener_episode).to.equal('listen2');
      expect(inserts['dt_impressions'][1].json.ad_id).to.equal(2);
      expect(inserts['dt_impressions'][2].json.listener_episode).to.equal('listen3');
