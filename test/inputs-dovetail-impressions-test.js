...
describe('dovetail-impressions', () => {
  ...
  it('partitions the table', () => {
    expect(impression.table({timestamp: 0})).to.equal('dt_impressions$19700101');
    expect(impression.table({timestamp: 1490827132000})).to.equal('dt_impressions$20170329');
    expect(impression.table({timestamp: 1490827132})).to.equal('dt_impressions$20170329');
    expect(impression.table({timestamp: 1490837132})).to.equal('dt_impressions$20170330');
  });
  ...
  it('formats table inserts', () => {
    return impression.format({requestUuid: 'the-uuid', timestamp: 1490827132999}).then(row => {
      ...
      expect(row.table).to.equal('dt_impressions$20170329');
  ...
  it('inserts impression records', () => {
    ...
    return impression2.insert().then(result => {
      ...
      expect(result[0].dest).to.equal('dt_impressions$20170329');
      expect(result[0].count).to.equal(2);
      expect(result[1].dest).to.equal('dt_impressions$20170330');
      expect(result[1].count).to.equal(1);
      expect(inserts['dt_impressions$20170329'].length).to.equal(2);
      expect(inserts['dt_impressions$20170329'][0].json.request_uuid).to.equal('the-uuid1');
      expect(inserts['dt_impressions$20170329'][1].json.request_uuid).to.equal('the-uuid4');
      expect(inserts['dt_impressions$20170330'].length).to.equal(1);
      expect(inserts['dt_impressions$20170330'][0].json.request_uuid).to.equal('the-uuid3');
    });
  });
