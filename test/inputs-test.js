...
describe("inputs", () => {
  it("handles unrecognized records", () => {
    const inputs = new BigqueryInputs([
      { type: "postbytes", listenerEpisode: "e1", timestamp: 0, download: {} },
      { type: "foobar", listenerEpisode: "fb", timestamp: 0, download: {} },
      { type: "postbytes", listenerEpisode: "e2", timestamp: 0, download: {} },
      { what: "ever" },
    ]);
  ...
  it("inserts all bigquery inputs", () => {
    ...
    sinon.stub(bigquery, "insert").callsFake((_ds, _tbl, rows) => Promise.resolve(rows.length));
    const inputs = new BigqueryInputs([
      { type: "postbytes", listenerId: "i1", timestamp: 0, impressions: [{}] },
      { type: "foobar", listenerId: "fb", timestamp: 0 },
      { type: "postbytes", listenerId: "d1", timestamp: 0, download: {} },
      { type: "postbytes", listenerId: "i2", timestamp: 999999, impressions: [{}] },
  ...
  it("inserts dynamodb inputs", async () => {
    ...
    const inputs = new DynamoInputs([
      { type: "postbytes", listenerEpisode: "le1", digest: "d1" },
  ...
  it("inserts pingback inputs", () => {
    ...
    const inputs = new PingbackInputs(
      [
        {
          type: "postbytes",
          listenerId: "i1",
