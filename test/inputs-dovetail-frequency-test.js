...
describe('dovetail-frequency', () => {
  ...
  it('recognizes frequency impressions', () => {
    expect(frequency.checkImpressions([{},{frequency: null},{frequency: ""},{frequency: "foo"}])).to.be.false;
  ...
  it('inserts impression records', () => {
    if (process.env.DDB_LOCAL) {
      ...
      sinon.stub(dynamo, 'updateItemPromise').callsFake(async params => {
        ...
        let exp = '';
        let imp = [`${current - 2600000}`, `${current - 2700000}`, `${current - 2800000}`, `${current - 2900000}`, `${current - 3000000}`, `${current - 3100000}`, `${current - 3200000}`, `${current - 3300000}`, `${current - 3400000}`, `${current - 3500000}`, `${current - 3600000}`];
    ...
    let frequency = new DovetailFrequency([
      ...
      },
      {
        ...
        impressions: [{ campaignId: 100, frequency: "1:1" }, { campaignId: 200, frequency: "1:1" }],
      },
      {
        type: 'postbytes',
